...
FILE* dataMem = fopen("DataMem.txt", "w+");

void insereDataMem(char* nome, int loc)
{
	fprintf(dataMem, "%s:%d\n", nome, loc);
}
						...
						else if(root->id_type == Call)
						{
							...
							if(root->codeGen != 1)
							{
								...
								if(t)
								{
									...
									sprintf(returnVal.contents.nome,"%s", "_TEMP_RET_");
									returnVal.escopo = root->scope;
	...
	switch(root->kind.stmt)
	{
		case AssignK:
		{
			if(root->codeGen!=1){
				...
				///se o lado direito eh uma funcao, o temp do return vai virar um _TEMP_RET_.
				if(root->child[0]->id_type == VectorPos) 
				{
					// printf("array\n");
					clearAddr(&tempEsq);
				...
				if(root->child[1]->id_type == Call && root->child[1]->kind.exp ==  IdK)
				{
					...
					clearAddr(&tempDir);
					sprintf(tempDir.contents.nome, "_TEMP_RET_");
					tempDir.kind = String;
				}
...
void tempDataMem()
{
	printf("Gerando Memoria de Dados Temporarios ...\n");
	//percorre o arquivo inteiro para descobrir onde tem os temporarios.
	//se for uma instrucao de load, so aloca espaco na memoria de dados para 
	//o primeiro operando
	//faz a insercao na tabela de simbolos.
	char stream[100], *ret;
	int tempCount = 0;
	char *op[4];
	char **entradaTabela;
	...
	{
		fgets(stream, 50, fp);
		if(strstr(stream, "TEMP") !=NULL){
			tempCount++;
		}
	}
	// printf("(tempDataMem)Numero de temporarios: %d\n", tempCount);
	entradaTabela = (char**)malloc(tempCount*sizeof(char*));
	for(int iterator = 0; iterator<tempCount; iterator++)
	{
		entradaTabela[iterator] = (char*)malloc(50*sizeof(char));
	}
	fseek(fp, 0, SEEK_SET);
	int i = 0;
	while(feof(fp)== 0)
	{
		fgets(stream, 50, fp);
		if(strstr(stream, "ld") !=NULL){
			//DIVIDE A LINHA NOS 4 CAMPOS (OPR,OP1,OP2,OP3)
			op[0] = strtok(stream, ",");
			op[1] = strtok(NULL, ",");
			op[2] = strtok(NULL, ",");
			op[3] = strtok(NULL, ",");
			
			if(strstr(op[1], "TEMP")!=NULL)
			{
				///insere(char* nome, Identifier tipo, ExpType dado, char* escopo, int valor, int linha, int varLoc)
				//conta quantos temporarios tem.
				strcpy(entradaTabela[i], op[1]);
				// printf("Entrada da tabela: %s\n", entradaTabela[i]);
				//insere(entradaTabela[i++],VarDecl, Integer, "global", 0, -1, dataMemVarLoc);
				//dataMemVarLoc++;
			}
		}
	}
	printf("Imprimindo a Tabela de Simbolos em Arquivo (tabela.txt)...\n");
	//imprimeTabela();
	printf("Definicao de memoria de Temporarios finalizada com sucesso!\n");
}
...
typedef enum {REG, NUM, MEM, LAB} Op_Type; //diz qual o tipo de dado que esta no no da lista. Serve para alocar a quantidade correta de bits para o dado.
...
void RemoveAssembly(Instruction_Assembly opcode, Operand val1, Operand val2, Operand val3, int line)
{
	Assembly * p;
	p = Codigo_Maquina;
	int flag_stop = 0;
...
int returnFree_Temp(){
	if(tabela_registradores[4].flag_utilizado == 1){
		return 4;
	}else{
		if(tabela_registradores[5].flag_utilizado == 1){
			return 5;
		}
	}
}
...
void Store_ALL_Reg_Table(){//TAD
	...
	Ini_Reg_Table();
	printf("Store_ALL_Reg_Table\n");
}

void Load_ALL_Mem_Table(){//TAD
	printf("Load_ALL_Mem_Table\n");
	Ini_Reg_Table();
	int reg;
	int flag_out = 0;
	int i = 0;
		...
		case 1:
		{
			...
			{	
				if(strstr(q.addr_01.contents.nome,"TEMP")) //q.addr_01 --> temporario
				{
					...
					strcpy(q.addr_01.contents.nome, copia);
		...
		case 2:
		{
			...
			{	
				if(strstr(q.addr_02.contents.nome,"TEMP")) //q.addr_02 --> temporario
				{
					...
					strcpy(q.addr_02.contents.nome, copia);
					auxiliar = strtok(copia, "_TEMP"); //auxiliar contem o valor do indice do temporario.
					auxiliarInt = atoi(auxiliar);
		...
		case 3:
		{
			...
			{	
				if(strstr(q.addr_03.contents.nome,"TEMP")) //q.addr_03 --> temporario
				{
					...
					strcpy(q.addr_03.contents.nome, copia);
					auxiliar = strtok(copia, "_TEMP"); //auxiliar contem o valor do indice do temporario.
					auxiliarInt = atoi(auxiliar);
					 AlocaComponente(TEMPORARIO, NULO, auxiliarInt, -1, NULL, NULL);
				}
...
void Load_Vetor_to_Reg(char* v, int position, char* desl, tipo_deslocamento type,int reg){//TAD
	...
	if(type==D_IMEDIATO){
		...
		if(type==D_VARIAVEL){
			...
			if(addr_base<256){
				Op1 = AlocaOperand(NUM,8, addr_base);
				...
				if (addr_base<65536){
					Op1 = AlocaOperand(NUM,16, addr_base);
					...
					Op1 = AlocaOperand(NUM,16, addr_base_shifted);
...
void Store_to_Vetor(char* v, int position, char* desl, tipo_deslocamento type,int reg){//TAD
	...
	if(type==D_IMEDIATO){
		...
		Op2 = AlocaOperand(NUM,16, addr_mem);
		Op3 = NULL;
			...
			if(field->type == TEMPORARIO){
				...
				if(field->type == IMEDIATO){
					if (field->value < 65536){
						Op1 = AlocaOperand(NUM,16, field->value);
						Op2 = AlocaOperand(REG,8, reg);
						Op3 = NULL;
...
void Store_Global_Reg_Table_nextScope(){//TAD
	...
	printf("Store_Global_Reg_Table_nextScope\n");
}
...
void Store_Global_Reg_Table_prevScope(){//TAD
	...
	printf("Store_Global_Reg_Table_prevScope\n");
}
...
Quad_List* Generate_Assembly_for_ARGS(Quad_List* code_int){
	printf("BEEEEEEEEEEEEEEEEEEEEGIN AAAAAARGS\n");
	Operand * Op1;
	Operand * Op2;
	Operand * Op3;
	...
	while(code_int->quad.op!=FUNCAL){
		...
		case BEGIN_ARGS:
			...
			Op1 = AlocaOperand(NUM,0, 0);
			InsereAssembly(I_POP_R,Op1,NULL,NULL);//POP.R 
			printf("POP.R \n");
		...
		case INTCODE_ADD:
			...
			printf("ADD,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_SUB:
			...
			printf("INTCODE_SUB,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_MUL:
			...
			printf("MUL,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_DIV:
			...
			printf("INTCODE_DIV,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_LT:
			...
			if (c->type == L_LABEL){
				...
				InsereAssembly(I_JPG,Op1,Op2,Op3);//JPG $2 3 c->value
				printf("JPG $2 $3 %d\n", c->value);
				tabela_registradores[2].flag_utilizado = 0;
				tabela_registradores[3].flag_utilizado = 0;
			}
			printf("INTCODE_LT,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_LEQ:
			...
			if (c->type == L_LABEL){
				...
				Op2 = AlocaOperand(REG,8, 3);
				Op3 = AlocaOperand(LAB,8, c->value);
				InsereAssembly(I_JPGE,Op1,Op2,Op3);//JPGE $2 3 c->value
				printf("JPGE $2 $3 %d\n", c->value);
				tabela_registradores[2].flag_utilizado = 0;
				tabela_registradores[3].flag_utilizado = 0;
			}
			printf("INTCODE_LEQ,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_GT:
			...
			if (c->type == L_LABEL){
				...
				Op2 = AlocaOperand(REG,8, 3);
				Op3 = AlocaOperand(LAB,8, c->value);
				InsereAssembly(I_JPL,Op1,Op2,Op3);//JPL $2 3 c->value
				printf("JPL $2 $3 %d\n", c->value);
				tabela_registradores[2].flag_utilizado = 0;
				tabela_registradores[3].flag_utilizado = 0;
			}
			printf("INTCODE_GT,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_GEQ:
			...
			if (c->type == L_LABEL){
				...
				Op2 = AlocaOperand(REG,8, 3);
				Op3 = AlocaOperand(LAB,8, c->value);
				InsereAssembly(I_JPLE,Op1,Op2,Op3);//JPLE $2 3 c->value
				printf("JPLE $2 $3 %d\n", c->value);
				tabela_registradores[2].flag_utilizado = 0;
				tabela_registradores[3].flag_utilizado = 0;
			}
			printf("INTCODE_GEQ,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_EQ:
			...
			printf("INTCODE_EQ,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_NEQ:
			...
			printf("INTCODE_NEQ,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_ASSIGN:
			...
			returnComponente(quad,1);
			Assign_Operand(c,3);
			printf("INTCODE_ASSIGN,Generate_Assembly_for_ARGS\n");
		break;
		case INTCODE_RET:
			...
			printf("POP.PC \n");
			printf("INTCODE_RET,Generate_Assembly_for_ARGS\n");
		break;
		case ARG:
			printf("ARG,Generate_Assembly_for_ARGS\n");
			returnComponente(quad,1);
			if (c->type == VARIAVEL){
		...
		case JMP:
			...
			Load_ALL_Mem_Table();
			printf("JMP,Generate_Assembly_for_ARGS\n");
		break;
		case LABEL:
			...
			Load_ALL_Mem_Table();
			printf("LABEL,Generate_Assembly_for_ARGS\n");
		break;
		}
		code_int = code_int->next;
	}
...
void Generate_Assembly(){
	...
	while(code_int!=NULL){
		...
		switch(quad.op){
		case BEGIN_ARGS:
			printf("BEGIN ARGS,Generate_Assembly\n");
			code_int = Generate_Assembly_for_ARGS(code_int->next);
			//FUNCAL
			Store_ALL_Reg_Table();
			Store_Global_Reg_Table_nextScope();
		...
		case INTCODE_DIV:
			...
			printf("INTCODE_DIV,Generate_Assembly\n");
		break;
		case INTCODE_LT:
			...
			printf("INTCODE_LT,Generate_Assembly\n");
		break;
		case INTCODE_LEQ:
			...
			printf("INTCODE_LEQ,Generate_Assembly\n");
		break;
		case INTCODE_GT:
			...
			printf("INTCODE_GT,Generate_Assembly\n");
		break;
		case INTCODE_GEQ:
			...
			printf("INTCODE_GEQ,Generate_Assembly\n");
		break;
		case INTCODE_EQ:
			returnComponente(quad,1);
			Load_Operand(c,2);
			...
			printf("INTCODE_EQ,Generate_Assembly\n");
		break;
		case INTCODE_NEQ:
			returnComponente(quad,1);
			Load_Operand(c,2);
			...
			printf("INTCODE_NEQ,Generate_Assembly\n");
		break;
		case INTCODE_ASSIGN:
			...
			returnComponente(quad,1);
			Assign_Operand(c,3);
			printf("INTCODE_ASSIGN,Generate_Assembly\n");
		break;
		case INTCODE_RET:
			...
			InsereAssembly(I_POP_PC,Op1,NULL,NULL);//POP.PC 
			printf("POP.PC \n");
			printf("INTCODE_RET,Generate_Assembly\n");
		break;
		case JMP:
		...
		case LABEL:
			...
			Load_ALL_Mem_Table();
			printf("LABEL,Generate_Assembly\n");
		break;
		case FUNCAL:
			printf("FUNCAL,Generate_Assembly\n");
			Store_ALL_Reg_Table();
			Store_Global_Reg_Table_nextScope();
			Op1 = AlocaOperand(NUM,32, reserved_mem_total);
			InsereAssembly(I_PUSH_R,Op1,NULL,NULL);//PUSH.R reserved_mem_total
	...
	initial_load_to_reg();
	printf("PROCESSANDO RESTO DO CODIGO\n");
	while(flag_out_assembly == 0){
		...
		case FUNDEF:
			...
			Create_Mem_Table(c->var);
			initial_load_to_reg();
			printf("FUNDEF,Generate_Assembly\n");
		break;
		case BEGIN_ARGS:
			...
			Store_Global_Reg_Table_prevScope();
			Load_ALL_Mem_Table();
			printf("BEGIN_ARGS,Generate_Assembly\n");
		break;
		case INTCODE_ADD:
...
void SymTabGen() {
	...
	if (semantical_error == 0) generateIntermCode();
	if (semantical_error == 0) Gen_Ass();
}
